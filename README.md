...
**就算鼓起勇气开了多个会话(Claude Code / Cursor 等),又会撞上新的五件事:**
...
这套总线的回答:**信息走文件不走人嘴、完成必须带证据、决策点必须人拍板、会腐烂的事实只放一处。** 上下文外置到文件之后,**任何会话都是可抛弃的**——崩了、满了、关了,新开一个跑一遍开工脚本就原地满血;这就是你敢开 N 个会话的底气。
...
2. **日常干活**:开三个会话,每个认领一个域,你只说三句话——
   - 对**产品**会话:「加个"月度报表"功能,拆一下」→ 它写需求、更看板,等你拍板(⛔Gate1)
   - 对**全栈**会话:「看板上月度报表该你了,开工」→ 它先跑 bus-check 对齐进度,读看板和契约,实现后留下 commit hash + 证据
   - 对**测试**会话:「验收月度报表」→ 它跑 E2E、对着设计稿截图走查,问题带图直接提
3. **你只拍板**:规格、设计(浏览器里点过真原型再批)、合并、上线四个关卡由你点头,每一锤自动记进决策台账。信息全在文件里流动,**你不用当传声筒,也不用怕关掉任何一个会话**。
...
flowchart LR
    ...
    Design -->|"design-preview.sh 真渲染<br>你点过原型再批 Gate2"| Deliver[全栈会话<br>实现+改契约]
    Deliver -->|reviewer只读核查| Verify[测试会话<br>E2E+带图走查]
...
├── lessons.md     # 14 条反模式(症状→根因→解药),每条都真实发生过
├── README.md      # 本文(给人读的介绍)
...
| 拍板 / 拍板台账 | 拍板 = 你做决定;台账 = 决策记录本(pm/decisions.md),全项目唯一登记处 |
...
*版本:v1(2026-06-10 首次蒸馏)· v1.1(2026-07-02 回灌三周实战:生产漂移检测机制 + 教训 11–13) · v1.2(2026-07-04 回灌:教训 14 UI 元注释复发 + §1.5 零元注释红线 + reviewer 清单第 6 条) · v1.3(2026-07-26 引导式 Bootstrap:自查代码+少量提问 + 三域改名 产品/全栈/测试(默认可自定义) + 名词白话表 + 简介重构:先问题→怎么用→举例 + 修 cp 丢点文件 + 指纹诚实边界)。后续项目再踩出新坑,记得回填 lessons.md——这份技能本身也适用"证据制+单点事实":教训只在 lessons.md 登记一处。*
