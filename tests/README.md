...
`check-docs.sh` uses only Python's standard library. It validates relative Markdown links, rejects non-portable internal citation markers, checks the bilingual README structure and required frontmatter, confirms that critical scaffold/CLI files exist, keeps package/changelog/example versions aligned, and prevents an unverified source version from appearing in executable install commands before independent distribution verification.
