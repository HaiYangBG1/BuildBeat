...
const HELP = `BuildBeat CLI v${CLI_VERSION} (Wave 1 source candidate)
...
only that prompt. upgrade, diff, and uninstall remain unavailable.
...
function parse(args) {
  if (args.length === 0) {
    return { command: "help", target: ".", json: false, dryRun: false, layout: null, yes: false };
  }
  const options = {
    command: args[0],
    target: ".",
...
export async function run(
  ...
  try {
    if (options.command === "doctor") {
      if (options.layout || options.dryRun || options.yes) {
