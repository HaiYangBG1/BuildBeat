...
**症状**:一个非生产健康聚合服务被 AI 会话命名为 `readmodel-nonprod`,域名也照此申请;所有者在会话里连问「这是干啥的」「名字怎么起的」「非生产?后面还要建生产?」「不做会影响什么」,最后自己改成 `platform-health`。同期「服务 4 小时自停」也是 worker 按契约自定,所有者见到才问「为什么需要 4 个小时」。
...
**症状**:两个子仓 58 个 Run 里 32 个被取消,多数是在等人批的状态挂满一天后被批量清掉,人批平均等 7～12 小时;另一边所有者守着一场部署战役时,在同一会话里问了十几次「半小时了正常吗」「十分钟了是卡住了吗」「你现在是在让 Codex 干活?」。事后清理时又发现 16 个终态 Run 的工作树无人收拾。
