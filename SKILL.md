...
>
> **CLI v0 只是只读证据入口,不是 Bootstrap 替身。** 有本仓源码且 Node.js 20+ 时,可先运行 `node bin/solobaton.js init <项目根> --dry-run --json`;存量项目改用 `adopt ... --dry-run --json`。把它的仓/部署标记/UI/测试/碰撞结果作为自查证据,随后仍要读代码、只问剩余问题并做一屏确认。v0 缺 `--dry-run` 必须拒绝,不得声称它已生成、升级或卸载项目。
...
> 有 CLI v0 时先跑 `node bin/solobaton.js adopt <项目根> --dry-run --json`:它默认规划紧凑布局并列出碰撞,但不会判断绞杀者边界、危险区或 L3 是否充分,这些仍按下方仪式核实和拍板。
...
>
> 仓库级 CLI 的命令、exit code、四类文件所有权、manifest schema 与未来 upgrade/uninstall fail-closed 规则见 [docs/CLI.md](docs/CLI.md)。CLI 与 Skill 共用同一协议,但职责不同:Skill 做代码级理解与人 Gate,CLI 只做确定性生命周期机械动作。
