...
**域回复格式** —— 面向用户收口、交接或回复明确检查点时,统一写「已做 → 未做 → 下一步」;中间进展和探索讨论不强制套模板。
...
只保留符合当前状态的一条。`已做`只写功能/业务结果,证据紧跟对应事项;多项共用时在列表末写一次「共同证据」。`未做`同时写原因,没有就写「无」;未验证边界也放这里。整个工作包已完成就写「下一棒:无」;下一棒按剩余目标决定,不固定按域流转。本域自己仍能在范围内推进时,继续做,不向用户伪求助或交棒。
