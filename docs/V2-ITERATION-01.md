...
> 状态：**执行中**——ChickAI 已在后续单独授权下完成两阶段生产发布，Tide 本地候选完成且未发布；第三个目标项目已选定为 `AI底座/底座`，但具体工作项尚未开跑。前两项仍由当前会话人工编排，未由 `pilot/loop.sh` 驱动，因此可作为 B 协议的真实使用证据，但尚不能形成 `V2-D2`
...
- [x] **T1（项目所有者，`V2-D1` + `V2-D1A`）**：首批工作项为 ChickAI 额度耗尽仍可登录（含 UI）与 Tide 悬浮球空闲收纳（小功能）；第三个目标项目已选定为 `AI底座/底座`，具体采用该项目下一项自然发生、已授权、低风险的非生产开发任务，不另造演示需求。
- [ ] **T2（每个执行工作项）**：ChickAI 与 Tide 均已准备独立 `ACCEPT_CMD`，在原基线先失败、实现后通过；第三项尚未冻结工作项与 oracle。
- [ ] **T3（每个执行工作项）**：ChickAI 与 Tide 均使用 `pilot/*` 专用 branch/worktree，intent、plan、protected paths 与验收 oracle 先独立提交；第三项尚未建立隔离现场。
...
- **AI 底座（第三目标项目，尚未开跑）**：当前工作树记录的是 legacy 手工迁移后的 BuildBeat `v1.21`，没有 schema 2 manifest；已提交基线仍是 `v1.20`，v1.21 迁移与其它项目改动尚未全部提交。项目为 meta + `ruoyi-ai` + `chickDEV` 等多仓结构。`WP-B1-AUTHZ` 的 exact backend/frontend candidate、独立 L3 和 reviewer closure 已形成，真实下一棒是产品 Gate2.5，但 `NOW/看板` 仍停在“等待全栈+测试”，可作为接力棒掉地与多仓证据绑定的回放材料。完整只读摸底见 [`pilot/targets/ai-platform-base.md`](../pilot/targets/ai-platform-base.md)。
...
- **执行口径**：B 方案从后续真实任务开始作为默认工作协议使用；不回放改写既有 candidate，也不专门造第三个演示任务。AI 底座下一项自然发生、已授权的低风险非生产开发任务必须从干净 oracle 进 loop，并补跑 F5/F6，之后再决定薄内核还是完整内核。
...
- AI 底座当前不做 in-place v2 升级：v2 尚无发布 tag/预发布包，目标项目又没有可机械升级的 manifest，且现有工作树/在途 Gate 不能被试点污染。第三轮采用旁路 `pilot/*` worktree；若工作项跨仓，所有 candidate 必须由同一 run 显式绑定。
