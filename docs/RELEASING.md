...
Release evidence at source package version `@haiyangbg/buildbeat@1.21.0`; latest independently verified BuildBeat npm distribution `@haiyangbg/buildbeat@1.20.0`, anchored by annotated tag `v1.20.0` at commit `5aaa9e8`, workflow run [32826832379](https://github.com/HaiYangBG1/BuildBeat/actions/runs/32826832379), and the matching [GitHub Release](https://github.com/HaiYangBG1/BuildBeat/releases/tag/v1.20.0). The 1.21 source candidate adds the standard domain-response contract without expanding the CLI command or safety boundary; it must not be described as a verified distribution until its own protected tag, OIDC workflow, registry identity, provenance, signatures, isolated install, and GitHub Release are read back. Exact 1.20 evidence, bootstrap behavior, and legacy deprecation are archived in [`WP4.3-RELEASE-EVIDENCE-2026-08-25.md`](WP4.3-RELEASE-EVIDENCE-2026-08-25.md). The legacy distribution remains `solobaton@1.16.3`; all three published legacy versions are retained and deprecated toward the scoped package.
...
10. The write-enabled first-screen command `npx --yes --package=@haiyangbg/buildbeat@latest buildbeat init my-project` is active because the exact 1.20 scoped artifact passed registry/provenance/signature/isolated-install/README readback. The old-name WP2.7 pilots remain historical compatibility evidence and do not redefine the scoped artifact.
