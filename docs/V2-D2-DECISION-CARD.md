...
- pilot-app、Tide、AI 试点工作区三次真实工作都没有由 `pilot/loop.sh` 自动激活，自动 Run ledger 为 `0/3`；三者均只能作为真实流程/负向证据，不能伪装成合格自动 Run。
...
保持 `V2-D2=PENDING`，等待 AI 试点工作区出现下一项自然、已授权、低风险的非生产开发任务，再要求它从干净 oracle 进入现有 `pilot/loop.sh`。不补造演示需求，不回放旧 candidate。
