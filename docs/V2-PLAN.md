...
`V2-D0=B` 采用的是本文整份执行基线，因此 D1–D6 已按下表推荐项同步生效，无需重复拍板；当前只剩 D7 / `V2-D1` 需要项目所有者提供真实工作项。
...
| D7 / `V2-D1` | **M-1 试点工作项（当前唯一外部阻塞项）** | 由项目所有者点名 1–3 个真实工作项：一个修 bug、一个小功能、一个含 UI；目标项目必须有回归命令，并能准备基线失败的独立验收命令 | 执行细则见 [`V2-ITERATION-01.md`](V2-ITERATION-01.md)；点名后才能实跑 |
