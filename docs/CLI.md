...
Status: **CLI v0 read-only preview** · package version `solobaton@1.16.2` · Node.js 20+ · zero third-party runtime dependencies.
...
A repository checkout may replace `npx --yes solobaton@1.16.2` with `node bin/solobaton.js`.
...
|---|---|---|
| Reproducible one-off run | `npx --yes solobaton@1.16.2 doctor <project>` | Read-only inspection; no persistent global CLI installation |
| Install a pinned global CLI | `npm install --global solobaton@1.16.2` | Installs only the package and executable |
